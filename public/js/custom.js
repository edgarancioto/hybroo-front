  ...
  switch (task) {
    case "functions_names":
      ...
      data.forEach(function (item) {
        $("#function_selected").append(
          '<option  value="' + item.id + '">' + item.name + "</option>"
        );
      });
      socket.send(
        JSON.stringify({ task: "functions_methods", params: "None" })
      );
      break;
...
function populateCardContainerTwo(value) {
  ...
  for (var i in labels) {
    $("#list").append(`
            <tr>
                <td class="font-weight-bold capitalize pr-3 vertical-top">${
                  labels[i]
                }</td>
...
function getDetails() {
  socket.send(
    JSON.stringify({
      ...
      params: { function_id: valueSelected },
    })
  );
  socket.send(
    JSON.stringify({
...
function sendData() {
  ...
  socket.send(
    JSON.stringify({ task: "functions_solver", params: { collectionData } })
  );
